...
class NewsReport(IReport):
    def __init__(self, news=None, prev=None, next=None, editable=False):
        ...
        for event in news:
            newsNode = self.CreateChild(viewNode, "news", time=time.ctime(event.timeStamp), author=event.authorName,
                                        id=event.timeStamp)
            try:
                ...
                self.CreateChild(newsNode, "div", **{"class": "news"}).appendChild(
                    factory.createTextNode(event.message))
...
class JuryQuestListReport(IReport):
    def __init__(self, questId, questName, questInfo, lang, user):
        ...
        questNode = self.CreateChild(viewNode, "quest", id=questId,
                                     got=questInfo["got"], done=questInfo["done"], tries=questInfo["tries"],
                                     last=questInfo.get("last", ""))
        ...
        self.CreateChild(questNode, "param", name="quest4close",
                         value="true" if (questInfo["available"] and questInfo["got"] == 0) else "false")
        ppndFlag = True
