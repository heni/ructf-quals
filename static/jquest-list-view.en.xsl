        ...
        <div class="page-inform">
            <p class="trail">
                <xsl:if test="$needOpen = 'true'">
                    <span class="link" onclick="openQuest('{$questId}');">open the quest</span>
                </xsl:if>
                <xsl:if test="$needClose = 'true'">
                    <span class="link" onclick="closeQuest('{$questId}');">close the quest</span>
                </xsl:if>
                <xsl:if test="$ppndMode = 'true'">
                    <span class="link" onclick="getAllSolutions('{$questId}');">all solutions</span>
                    <span class="select-link">unchecked</span>
                </xsl:if>
                <xsl:if test="$ppndMode != 'true'">
                    <span class="select-link">all solutions</span>
                    <span class="link" onclick="getUncheckedSolutions('{$questId}');">unchecked</span>
                </xsl:if>
                <a href="quest">back</a>
            </p>
            <p class="header">
