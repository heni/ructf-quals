...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="view[@type = 'jury-quest-get']">
        ...
        <div class="page-inform">
            <p class="trail"><a href="quest">back</a></p>
            <p class="header"><xsl:value-of select="$questName" /></p>
            <xsl:apply-templates select="quest/view" />
            <p class="small-header"> 
                Solution by <span class="team-select"><xsl:value-of select="solution/@team" /></span>
            </p>
            <span class="time">(<xsl:value-of select="solution/@time" />)
                <xsl:choose>
                    <xsl:when test="solution/@status = 'accepted'"> solution accepted </xsl:when>
                    <xsl:when test="solution/@status = 'rejected'"> solution rejected </xsl:when>
                    <xsl:when test="solution/@status = 'postponed'"> solution delayed </xsl:when>
                </xsl:choose>
            ...
            <xsl:if test="solution/@file">
                <a href="{solution/@file}">Materials from the team</a>
            </xsl:if>
            <div id="btn-panel">
